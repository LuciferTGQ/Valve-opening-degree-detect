"""
对增强后的 side 图片做 CV 裁剪，提取阀门区域
用于训练新的 CV+CNN 融合模型

流程：遍历 data_augmented/side/ 的每张增强图 →
      _crop_valve_region() 裁剪 → 保存到 data_cropped/side/
      裁剪失败则跳过（不保留原图）
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description='CV 裁剪增强后的 side 图片')
    parser.add_argument('--input', default='origin data/data_augmented/side', help='增强数据目录')
    parser.add_argument('--output', default='data_cropped/side', help='裁剪输出目录')
