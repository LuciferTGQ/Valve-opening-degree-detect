...
def main():
    ...
    parser = argparse.ArgumentParser(description='按角度均衡增强原始 top 图片')
    parser.add_argument('--input', default='origin data/top', help='原始数据目录')
    parser.add_argument('--output', default='data_augmented_balanced/top', help='增强输出目录')
    parser.add_argument('--target', type=int, default=150, help='每个角度目标数量')
    ...
    print(f"输入: {len(files)} 张原始图片, {len(angle_files)} 个角度")
    ...
    # 打印摘要
    small_angles = sum(1 for _, (_, aug, _) in plan.items() if aug == args.max_augment)
    print(f"增强计划摘要:")
    print(f"  达到最大增强倍数的角度: {small_angles} 个")
    print(f"  预计总量: {total_target} 张\n")
    ...
    for angle, (file_list, augment_per_img, actual) in sorted(plan.items()):
        ...
        print(f"  {angle:>6.1f}°: {len(file_list)}张 × {augment_per_img}x → {actual}张")
